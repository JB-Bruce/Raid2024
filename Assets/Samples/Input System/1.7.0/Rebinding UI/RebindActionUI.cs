...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void ResetBinding(InputAction action, int bindingIndex)
        {
            ...
            {
                int compositeStartIndex = bindingIndex;
                while (compositeStartIndex > 0 && !action.bindings[compositeStartIndex].isComposite)
                {
                    compositeStartIndex--;
                }

                for (int i = compositeStartIndex; i < action.bindings.Count; i++)
                {
                    InputBinding compositeBinding = action.bindings[i];

                    if (i != compositeStartIndex &&  !compositeBinding.isPartOfComposite)
                    {
                        break;
                    }

                    if (compositeBinding.overridePath == newBinding.path)
                    {
                        action.ApplyBindingOverride(i, oldOverridePath);
                        break;
                    }
                }
            }
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            //Disables the action before use to prevent errors
            action.Disable();
        ...
        private bool CheckDuplicateBindings(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            // Check against all bindings in the action map for general duplicates
            foreach (InputBinding binding in action.actionMap.bindings)
            {
                ...
                {
                    Debug.Log("Duplicate binding found: " + newBindingPath);
                    return true;
                }
            }

            // Additional check if the new binding is part of a composite or if checking all composite parts
            if (newBinding.isPartOfComposite || allCompositeParts)
            {
                // Find the start of the composite
                int compositeStartIndex = bindingIndex;
                while (compositeStartIndex > 0 && !action.bindings[compositeStartIndex].isComposite)
                {
                    compositeStartIndex--;
                ...
                // Iterate through the composite parts
                for (int i = compositeStartIndex; i < action.bindings.Count; i++)
                {
                    InputBinding binding = action.bindings[i];

                    // Stop when the end of the composite is reached
                    if (i != compositeStartIndex && !binding.isPartOfComposite)
                    {
                        break;
                    }

                    // Skip the current binding being checked
                    if (i == bindingIndex)
                    {
                        continue;
                    }

                    // Check if the effective path matches within the composite parts
                    if (binding.effectivePath == newBindingPath)
                    {
                        Debug.Log("Duplicate composite part binding found: " + newBindingPath);
                        return true;
                    }
                }
            }
